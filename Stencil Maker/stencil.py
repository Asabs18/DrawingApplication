...
def sizePoint(size, width):
	if size == True:
		...
		text = font.render(str(width), True, white, barColor) 
		textRect = text.get_rect()
		textRect.center = (1100, 20)
		screen.blit(text, textRect)
		if pygame.mouse.get_pressed() == (1,0,0):
			if plusBtn.rect.collidepoint(pygame.mouse.get_pos()):
				if width <= 99:
					width += 1
				time.sleep(.05)
			elif minusBtn.rect.collidepoint(pygame.mouse.get_pos()):
				if width > 1:
					width -= 1
				time.sleep(.05)
...
def tools(drawColor):
	...
	if pygame.mouse.get_pressed() == (1,0,0):
		if clearBtn.rect.collidepoint(pygame.mouse.get_pos()):
			...
			toolButtons(False, False, "null", False, False)
			if size == True:
				size = False
			else:
				size = True
...
def position(screen):
	...
	text = font.render(str(pos), True, black, barColor) 
	textRect = text.get_rect()
	textRect.center = (30, 125)
	screen.blit(text, textRect)
...
plusBtn = MyButton(screen, "+", 1150, 12.5, 50, 50, 0, 255, 0)
minusBtn = MyButton(screen, "-", 1050, 12.5, 50, 50, 255, 0, 0)
